...
Музей процессоров AU-team при РГУ нефти и газа имени И.М. Губкина на протяжении нескольких лет активно собирает коллекцию процессоров – от классических Intel и AMD до экзотических экземпляров вроде Transmeta. С полной коллекцией можно ознакомиться на нашем официальном сайте https://au-team.ru.
...
**Процессор:** Intel i486 DX2-50 (маркировка A80486DX2-50, S-spec SL503, 2595-я партия)  
...
Это означает, что перед установкой Windows 95 необходимо сначала установить MS-DOS 6.22. Для молодого поколения DOS – это экзотика, но в своё время она была абсолютным стандартом. MS-DOS, PC-DOS (IBM), DR-DOS, FreeDOS – все эти системы управляли миллионами компьютеров через командную строку.
...
#### Что такое XT-IDE?
XT-IDE – это плата расширения для шины ISA, разработанная энтузиастами ретрокомпьютинга для подключения современных IDE-устройств к древним ПК. Название отсылает к IBM PC XT (1983) – компьютеру, на котором впервые появилась шина ISA, но который ещё не имел стандарта IDE.
Плата GlitchWorks – это современная реплика оригинального проекта XT-IDE Universal BIOS, только с улучшенной элементной базой. На плате установлена микросхема EEPROM (электрически стираемая память), в которую прошита программная прослойка – Option ROM.
Как это работает?
При загрузке системы BIOS материнской платы опрашивает все ISA-слоты на наличие плат расширения с собственными ROM-микросхемами. Если находит – передаёт им управление для инициализации. XT-IDE перехватывает этот момент и:

Инициализирует свой IDE-контроллер
Определяет подключенные устройства (в нашем случае – CF-карта)
Регистрирует их как доступные диски в системе
Возвращает управление основному BIOS
